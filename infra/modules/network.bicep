...
resource appGatewayNsg 'Microsoft.Network/networkSecurityGroups@2024-01-01' = {
  name: '${projectName}-${toLower(environment)}-appgw-nsg-${uniqueString(resourceGroup().id, 'appgw-nsg')}'
  location: location
  properties: {
    securityRules: [
      {
...
resource containerAppNsg 'Microsoft.Network/networkSecurityGroups@2024-01-01' = {
  name: '${projectName}-${toLower(environment)}-container-nsg-${uniqueString(resourceGroup().id, 'container-nsg')}'
  location: location
  properties: {
    securityRules: [
      {
...
resource databaseNsg 'Microsoft.Network/networkSecurityGroups@2024-01-01' = {
  name: '${projectName}-${toLower(environment)}-db-nsg-${uniqueString(resourceGroup().id, 'db-nsg')}'
  location: location
  properties: {
    securityRules: [
      {
...
resource vnet 'Microsoft.Network/virtualNetworks@2024-01-01' = {
  name: '${projectName}-${toLower(environment)}-vnet-${uniqueString(resourceGroup().id, 'vnet')}'
  location: location
  properties: {
    addressSpace: {
      addressPrefixes: [
